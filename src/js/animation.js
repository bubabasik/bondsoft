...
$(document).ready(function() {
	...
	/* строки первого экрана*/
	$(function(){
		const title = document.querySelector('.first-screen-old-anim .top__title');
		if(!title || windowWidth < 1200) {return false;}
		...
		.to($('.first-screen-old-anim .top__title > span:nth-child(2n+2)'), { x: 480 }, '<') 
		.to($('.first-screen-old-anim .top__title > span:nth-child(2n+1):not(:first-child)'), { x: 180 }, '<') 
	})

	/* второй экран */
	$(function(){ 
		const line = document.querySelector('.topline__cont');
		...
		let tl = gsap.timeline({
			scrollTrigger: {
				...
				onUpdate: self  => {
					let h = self.progress * 100;
					gsap.set(inner, {
						right: self.progress * $(window).outerHeight()
					})
				}
			}
		});
